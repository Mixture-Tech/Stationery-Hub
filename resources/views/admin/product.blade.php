       ...
       <!-- Form tìm kiếm -->
        <div class="mb-4">
            <form action="{{ route('admin.products') }}" method="GET" class="flex items-center flex-wrap gap-2">
                <input type="text" name="search" placeholder="Tìm kiếm theo mã sản phẩm, tên sản phẩm..." 
        ...
        @if($products->isEmpty())
            <p class="text-gray-500 text-center">Không có sản phẩm nào để hiển thị.</p>
        @else
                    ...
                    <tbody>
                        @foreach($products as $product)
                            <tr class="hover:bg-gray-100">
                                <td class="border border-gray-300 px-4 py-2 text-center">{{ $product->id_product }}</td>
                                <td class="border border-gray-300 px-4 py-2 text-center">{{ $product->name }}</td>
                                <td class="border border-gray-300 px-4 py-2 text-center">{{ optional($product->category)->name_category ?? 'Không có danh mục' }}</td>
                                <td class="border border-gray-300 px-4 py-2 text-center">{{ optional(optional($product->category)->parent)->name_parent ?? 'Không có danh mục cha' }}</td>
                                <td class="border border-gray-300 px-4 py-2 text-center">{{ number_format($product->price, 2, ',', '.') }} VNĐ</td>
                                <td class="border border-gray-300 px-4 py-2 text-center">{{ $product->discount_price ? number_format($product->discount_price, 2, ',', '.') . ' VNĐ' : 'N/A' }} </td>
                                <td class="border border-gray-300 px-4 py-2 text-center">{{ $product->nums }}</td>
